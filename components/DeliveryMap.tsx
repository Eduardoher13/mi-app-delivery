...
// En Android usamos PROVIDER_GOOGLE (requiere API key en app.json y, si el mapa
// sale en blanco en Expo Go, un dev build: npx expo run:android).
// En iOS dejamos el proveedor por defecto (Apple Maps), que no requiere key.
...
export function DeliveryMap({
  ...
  useEffect(() => {
    const coords = polylineCoordinates.length > 0
      ? polylineCoordinates
      : [pickup, delivery];
  ...
  return (
    <View style={[{ overflow: 'hidden' }, style]}>
      <MapView ref={mapRef} {...mapProps}>
        ...
        <Marker coordinate={pickup} title="Tienda" pinColor="#00A878" />

...
export function BannerPromo({
  ...
  return (
    <View className="overflow-hidden rounded-2xl bg-[#1A202C] p-5">
      <View className="flex-row items-center justify-between">
        <View className="flex-1 pr-4">
          <Text className="mb-1 text-xs font-bold tracking-widest text-[#00A878]">
            {badge}
          </Text>
          <Text className="text-xl font-black italic text-white">{title}</Text>
          <Text className="mt-2 text-sm text-white/80">{subtitle}</Text>
          <Pressable
            className="mt-4 self-start rounded-lg bg-[#00A878] px-5 py-2.5"
            onPress={onPress}
          >
        ...
        <View className="h-20 w-20 items-center justify-center rounded-full bg-white/10">
          <Ionicons name="construct" size={40} color="#00A878" />
        </View>
      </View>
    </View>
  );
